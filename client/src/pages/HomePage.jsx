...
import ZenVoiceAccountPrompt from "../components/zenvoice/ZenVoiceAccountPrompt";
...
const HomePage = () => {
    ...
    useEffect(() => {
        ...
        const handlePrompt = () => setShowAccountPrompt(true);
        window.addEventListener('show-zenvoice-prompt', handlePrompt);
        
        return () => {
            window.removeEventListener('resize', handleResize);
            window.removeEventListener('show-zenvoice-prompt', handlePrompt);
        };
    }, []);
    ...
    return isMobile ? (
        <>
            {!token && <GuestOverlay />}
            {showAccountPrompt && <ZenVoiceAccountPrompt onClose={() => setShowAccountPrompt(false)} />}
            <BottomSheetLayout />
        </>
    ) : (
