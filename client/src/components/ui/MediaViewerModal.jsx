...
const MediaViewerModal = ({ url, type, username, onClose }) => {
    // Close on Escape key
    useEffect(() => {
    ...
    return (
        <div className="media-viewer-overlay" onClick={onClose} style={{
            ...
            <div className="media-viewer-content" onClick={(e) => e.stopPropagation()} style={{
                ...
                {type === 'video' ? (
                    <video src={url} controls autoPlay style={{ maxWidth: '100%', maxHeight: '90vh', borderRadius: '8px', outline: 'none' }} />
                ) : (
                    <img src={url} alt="Viewed media" style={{ maxWidth: '100%', maxHeight: '90vh', borderRadius: '8px', objectFit: 'contain' }} />
                )}
            </div>
        </div>
    );
